...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  2 14:29:23 2023
...
# Proton energies (MeV)
proton_energies = np.linspace(1, 250, 250)
...
# Let's Normalize the data at 226 MeV
energy_normalization_index = np.abs(proton_energies - 226).argmin()
normalization_value = stopping_power_ratios[energy_normalization_index]
stopping_power_ratios_normalized = [ratio / normalization_value for ratio in stopping_power_ratios]

# Let's Plot the stopping power ratio vs proton energy
plt.figure()
plt.plot(proton_energies, stopping_power_ratios_normalized, label='Al2O3/H2O Stopping Power Ratio (Normalized)')
plt.xlabel('Proton Energy (MeV)')
plt.ylabel('Normalized Stopping Power Ratio')
plt.title('Normalized Stopping Power Ratio of Al2O3 to H2O for Protons')
plt.legend()
plt.grid()
plt.show()
